...
//   ALI_SMS_SIGN_NAME      - 短信签名（需审核通过，如"月月贷"）
//   ALI_SMS_TEMPLATE_CODE  - 短信模板编号（如 SMS_xxxxx）
// 模板示例：您的验证码是${code}，${minutes}分钟内有效。请勿泄露！
...
// ========== 阿里云短信发送（HMAC-SHA1 签名）==========
function aliyunSendSms(phone, code) {
      ...
      res.on('end', () => {
        try {
          ...
          if (result.Code === 'OK') {
            ...
            console.error(`  ❌ 阿里云短信失败: ${result.Code} - ${result.Message}`);
            reject(new Error(result.Message || '短信发送失败'));
          }
        } catch (e) {
...
app.post('/api/send-code', (req, res) => {
  ...
  aliyunSendSms(phone, code)
    .then(() => res.json({ ok: true }))
    .catch(err => {
      // 短信发送失败时仍返回 ok（验证码已存入，可控制台查看）
      res.json({ ok: true, msg: '短信接口异常，请联系管理员' });
    });
});
